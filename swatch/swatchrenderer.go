...
type SwatchRenderer struct {
	...
	//parent points to swatch that created this renderer to determine if something is selected in refresh function
	parent *Swatch
}
...
// determine where the swatch is place in layout
...
func (renderer *SwatchRenderer) Refresh() {
	...
	if renderer.parent.Selected {
		...
		//pick a new color, in this case, white
		renderer.square.StrokeColor = color.NRGBA{255, 255, 255, 255}
		//update existing render objects
